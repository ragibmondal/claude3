...
class ClaudeLlm:
     def __init__(self, client, user_input):
         ...
         self.cost_df = pd.DataFrame(columns=["Model", "Input Tokens", "Output Tokens", "Input Cost", "Output Cost", "Total Cost", "Total_JPY conversion", "Processing time "])

     def convert_usd_to_jpy(self, usd_amount):
         c = CurrencyRates()
         try:
             rate = c.get_rate('USD', 'JPY')
             jpy_rate = (f"Exchange rate: {rate:.2f}JPY/USD")
             return usd_amount * rate, jpy_rate
         except Exception as e:
             rate = 150 # Exchange rate to use as fallback
             jpy_rate = (f"Exchange rate: {rate:.2f} yen/dollar assumption")
             return usd_amount * rate, jpy_rate
     ...
     def generate_responses(self, model_name):
         ...
         new_row = {
             ...
             "Total_JPY conversion": f"\{jpy_total_cost:.3f}",
             "Processing time": f"{response_time:.2f} seconds"
         }
